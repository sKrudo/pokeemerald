...
static const u16 sPkmnData_Tilemap[] =
        {
                0x0101, 0x0102, 0x0103, 0x0104, 0x0105, 0x0106, 0x0107, 0x0108, 0x0111, 0x0112, 0x0113, 0x0114, 0x0115,
                0x0116, 0x0117, 0x0118,
                0x2101, 0x2102, 0x2103, 0x2104, 0x2105, 0x2106, 0x2107, 0x2108, 0x2111, 0x2112, 0x2113, 0x2114, 0x2115,
                0x2116, 0x2117, 0x2118,
        };
...
static const u16 sCloseBoxButton_Tilemap[] =
        {
                0x014c, 0x014d, 0x014e, 0x014f, 0x0170, 0x0171, 0x0172, 0x0173, 0x0174, 0x015c, 0x015d, 0x015e, 0x015f,
                0x0180, 0x0181, 0x0182,
                0x0183, 0x0184, 0x0175, 0x0176, 0x0177, 0x0178, 0x0179, 0x017a, 0x017b, 0x017c, 0x017d, 0x0185, 0x0186,
                0x0187, 0x0188, 0x0189,
...
void DrawTextWindowAndBufferTiles(const u8 *string, void *dst, u8 zero1, u8 zero2, s32 bytesToBuffer) {
    ...
    if (remainingBytes > 0) CpuFill16((zero2 << 4) | zero2, dst, (u32) (remainingBytes) * 0x100);
...
// Unused
static void
UnusedWriteRectCpu(u16 *dest, u16 dest_left, u16 dest_top, const u16 *src, u16 src_left, u16 src_top, u16 dest_width,
                   u16 dest_height, u16 src_width) {
    u16 i;
...
static void UnusedWriteRectDma(u16 *dest, u16 dest_left, u16 dest_top, u16 width, u16 height) {
    ...
    dest += dest_top * 0x20 + dest_left;
    width *= 2;
    for (i = 0; i < height; dest += 0x20, i++) Dma3FillLarge16_(0, dest, width);
}
...
static void Task_PCMainMenu(u8 taskId) {
    ...
    switch (task->tState) {
        case STATE_LOAD:
            ...
            AddTextPrinterParameterized2(0, 1, sMainMenuTexts[task->tSelectedOption].desc, TEXT_SPEED_FF, NULL, 2, 1,
                                         3);
            ...
            switch (task->tInput) {
                case MENU_NOTHING_CHOSEN:
                    ...
                    if (task->tSelectedOption != task->tNextOption) {
                        ...
                        FillWindowPixelBuffer(0, PIXEL_FILL(1));
                        AddTextPrinterParameterized2(0, 1, sMainMenuTexts[task->tSelectedOption].desc, 0, NULL, 2, 1,
                                                     3);
                    }
                    break;
                case MENU_B_PRESSED:
...
static void ChooseBoxMenu_PrintInfo(void) {
    ...
    CpuCopy32((void *) winTileData,
              (void *) OBJ_VRAM0 + 0x100 + (GetSpriteTileStartByTag(sChooseBoxMenu->tileTag) * 32), 0x400);
...
static void StartDisplayMonMosaicEffect(void) {
    ...
    if (sStorage->displayMonSprite) {
        ...
        SetGpuReg(REG_OFFSET_MOSAIC,
                  (sStorage->displayMonSprite->data[0] << 12) | (sStorage->displayMonSprite->data[0] << 8));
    }
}
    ...
    for (i = 0; i < IN_BOX_ROWS; i++) {
        for (j = 0; j < IN_BOX_COLUMNS; j++) {
            ...
            if (species != SPECIES_NONE) {
                ...
                sStorage->boxMonsSprites[count] = CreateMonIconSprite(species, personality, 8 * (3 * j) + 100,
                                                                      8 * (3 * i) + 44, 2, 19 - j);
            } else {
...
static void CreateBoxMonIconAtPos(u8 boxPosition) {
    ...
    if (species != SPECIES_NONE) {
        ...
        sStorage->boxMonsSprites[boxPosition] = CreateMonIconSprite(species, personality, x, y, 2,
                                                                    19 - (boxPosition % IN_BOX_COLUMNS));
        if (sStorage->boxOption == OPTION_MOVE_ITEMS)
            sStorage->boxMonsSprites[boxPosition]->oam.objMode = ST_OAM_OBJ_BLEND;
    }
}
...
static void MovePartySprites(s16 yDelta) {
    ...
    for (i = 0; i < PARTY_SIZE; i++) {
        if (sStorage->partySprites[i] != NULL) {
            ...
            posY = sStorage->partySprites[i]->y + sStorage->partySprites[i]->y2 +
                   sStorage->partySprites[i]->centerToCornerVecY;
            posY += 16;
            if (posY > 192)
                sStorage->partySprites[i]->invisible = TRUE;
            else
...
static void Task_InitBox(u8 taskId) {
    ...
    switch (task->tState) {
        case 0:
            ...
            sStorage->bg2_X = 0;
            task->tDmaIdx = RequestDma3Fill(0, sStorage->wallpaperBgTilemapBuffer,
                                            sizeof(sStorage->wallpaperBgTilemapBuffer), 1);
            break;
        case 1:
...
static bool32 WaitForWallpaperGfxLoad(void) {
    ...
    if (sStorage->wallpaperTiles != NULL) FREE_AND_SET_NULL(sStorage->wallpaperTiles);
...
static void InitCanReleaseMonVars(void) {
    ...
    sStorage->restrictedReleaseMonMoves = GetMonData(&sStorage->tempMon, MON_DATA_KNOWN_MOVES,
                                                     (u8 *) sStorage->restrictedMoveList);
...
static s8 RunCanReleaseMon(void) {
    ...
    switch (sStorage->releaseCheckState) {
        ...
        case 1:
            ...
            // moves the release Pokémon knows
            for (i = 0; i < IN_BOX_COUNT; i++) {
                knownMoves = GetAndCopyBoxMonDataAt(sStorage->releaseCheckBoxId, sStorage->releaseCheckBoxPos,
                                                    MON_DATA_KNOWN_MOVES, (u8 *) sStorage->restrictedMoveList);
...
static bool8 IsRemovingLastPartyMon(void) {
    if (sCursorArea == CURSOR_AREA_IN_PARTY && !sIsMonBeingMoved &&
...
static void SetDisplayMonData(void *pokemon, u8 mode) {
    ...
    if (mode == MODE_PARTY) {
        ...
        if (sStorage->displayMonSpecies != SPECIES_NONE) {
            ...
            sStorage->displayMonPalette = GetMonSpritePalFromSpeciesAndPersonality(sStorage->displayMonSpecies, otId,
...
static void MultiMove_UpdateSelectedIcons(void) {
    s16 columnChange = (abs(sMultiMove->fromColumn - sMultiMove->cursorColumn)) -
                       (abs(sMultiMove->fromColumn - sMultiMove->toColumn));
...
static bool8 UpdateItemInfoWindowSlideIn(void) {
    ...
    for (i = 0; i < pos; i++)
        WriteSequenceToBgTilemapBuffer(0,
                                       GetBgAttribute(0, BG_ATTR_BASETILE) + 0x14 + sStorage->itemInfoWindowOffset + i,
                                       i, 13, 1, 7, 15, 21);
...
void
CreateBoxMonAt(u8 boxId, u8 boxPosition, u16 species, u8 level, u8 fixedIV, u8 hasFixedPersonality, u32 personality,
...
// Unused
static bool8
UnkUtil_CpuAdd(u8 *dest, u16 dLeft, u16 dTop, const u8 *src, u16 sLeft, u16 sTop, u16 width, u16 height, u16 unkArg) {
    struct UnkUtilData *data;
