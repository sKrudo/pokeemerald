...
static bool8
ListMenuChangeSelection(struct ListMenu *list, bool8 updateCursorAndCallCallback, u8 count, bool8 movingDown);
...
// unused
s32
ListMenuTestInput(struct ListMenuTemplate *template, u32 scrollOffset, u32 selectedRow, u16 keys, u16 *newScrollOffset,
                  u16 *newSelectedRow) {
...
void ScriptMenu_ScrollingMultichoice(struct ScrollingListMenu *menu) {
    ...
    for (i = 0; i < menu[setId].count; i++)
        width = DisplayTextAndGetWidth(menu[setId].set[i].name, width);

    width = ConvertPixelWidthToTileWidth(width);
    left = ScriptMenu_AdjustLeftCoordFromWidth(left, width);
    windowId = CreateWindowFromRectBaseBlock(left, top, width, maxShowed * 2, 0x6D);
    ...
    gMultiuseListMenuTemplate.items = menu[setId].set;
