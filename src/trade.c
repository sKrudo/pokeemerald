    ...
    switch (gMain.state) {
        ...
        case 7:
            ...
            for (i = 0; i < sTradeMenuData->partyCounts[TRADE_PARTNER]; i++) {
                ...
                sTradeMenuData->partySpriteIds[TRADE_PARTNER][i] = CreateMonIcon(
                        GetMonData(mon, MON_DATA_SPECIES2, NULL),
                        SpriteCB_MonIcon,
        ...
        case 9:
            ...
            break;
        case 10:
            DrawTextWindowAndBufferTiles(gSaveBlock2Ptr->playerName, sMenuTextTileBuffers[GFXTAG_PLAYER_NAME_L], 0, 0,
                                         3);
            id = GetMultiplayerId();
            DrawTextWindowAndBufferTiles(gLinkPlayers[id ^ 1].name, sMenuTextTileBuffers[GFXTAG_PARTNER_NAME_L], 0, 0,
                                         3);
            DrawTextWindowAndBufferTiles(sTradeActionTexts[TRADE_TEXT_CANCEL], sMenuTextTileBuffers[GFXTAG_CANCEL_L], 0,
                                         0, 2);
        ...
        case 13:
            ...
            sTradeMenuData->cursorSpriteId = CreateSprite(&sSpriteTemplate_Cursor, sTradeMonSpriteCoords[0][0] * 8 + 32,
    ...
    switch (gMain.state) {
        ...
        case 13:
            ...
            sTradeMenuData->cursorSpriteId = CreateSprite(&sSpriteTemplate_Cursor,
                                                          sTradeMonSpriteCoords[sTradeMenuData->cursorPosition][0] * 8 +
                                                          32,
        ...
        case 20:
            if (_GetBlockReceivedStatus() == 3) {
                Trade_Memcpy(sTradeMenuData->giftRibbons, gBlockRecvBuffer[id ^ 1],
                             sizeof(sTradeMenuData->giftRibbons));
                TradeResetReceivedFlags();
                sTradeMenuData->bufferPartyState++;
            }
            break;
...
static void TradeMenuProcessInput(void) {
    ...
    if (JOY_NEW(A_BUTTON)) {
        ...
        if (sTradeMenuData->cursorPosition < PARTY_SIZE) {
            ...
            DrawBottomRowText(sTradeActionTexts[TRADE_TEXT_CANCEL_TRADE],
...
static void TradeMenuChooseMon(void) {
    ...
    DrawBottomRowText(sTradeActionTexts[TRADE_TEXT_CHOOSE_MON],
                      (void *) (OBJ_VRAM0 + sTradeMenuData->bottomTextTileStart * 32), 24);
}
...
static void TradeMenuShowMonSummaryScreen(void) {
    if (!gPaletteFade.active) {
        // Player's party
        if (sTradeMenuData->cursorPosition < PARTY_SIZE)
            ShowPokemonSummaryScreen(SUMMARY_MODE_LOCK_MOVES, gPlayerParty, sTradeMenuData->cursorPosition,
                                     sTradeMenuData->partyCounts[TRADE_PLAYER] - 1, CB2_ReturnToTradeMenu);
            // Partner's party
        else
            ShowPokemonSummaryScreen(SUMMARY_MODE_LOCK_MOVES, gEnemyParty, sTradeMenuData->cursorPosition - PARTY_SIZE,
...
static void DrawTradeMenuParty(u8 whichParty) {
    ...
    switch (sTradeMenuData->drawPartyState[whichParty]) {
        ...
        case 3:
            ...
            CopyBgTilemapBufferToVram(1);
            gSprites[sTradeMenuData->partySpriteIds[0][partyIdx + (selectedMonParty * PARTY_SIZE)]].x =
                    (sTradeMonSpriteCoords[selectedMonParty * PARTY_SIZE][0]
                     + sTradeMonSpriteCoords[selectedMonParty * PARTY_SIZE + 1][0]) / 2 * 8 + 14;
            gSprites[sTradeMenuData->partySpriteIds[0][partyIdx + (selectedMonParty * PARTY_SIZE)]].y =
                    (sTradeMonSpriteCoords[selectedMonParty * PARTY_SIZE][1] * 8) - 12;
...
static void ResetTradeMenuPartyPositions(u8 whichParty) {
    ...
    for (i = 0; i < sTradeMenuData->partyCounts[whichParty]; i++) {
        ...
        gSprites[sTradeMenuData->partySpriteIds[whichParty][i]].x =
                sTradeMonSpriteCoords[(whichParty * PARTY_SIZE) + i][0] * 8 + 14;
        gSprites[sTradeMenuData->partySpriteIds[whichParty][i]].y =
...
int GetUnionRoomTradeMessageId(struct GFtgtGnameSub rfuPlayer, struct GFtgtGnameSub rfuPartner, u16 playerSpecies2,
                               u16 partnerSpecies, u8 requestedType, u16 playerSpecies, u8 isEventLegal) {
...
static void LoadTradeMonPic(u8 whichParty, u8 state) {
    ...
    switch (state) {
        case 0:
            ...
            if (whichParty == TRADE_PLAYER)
                HandleLoadSpecialPokePic_2(&gMonFrontPicTable[species], gMonSpritesGfxPtr->sprites.ptr[1], species,
                                           personality);
            else
                HandleLoadSpecialPokePic_DontHandleDeoxys(&gMonFrontPicTable[species],
                                                          gMonSpritesGfxPtr->sprites.ptr[whichParty * 2 + 1], species,
                                                          personality);
    ...
    switch (sTradeData->state) {
        ...
        case TS_STATE_BYE_BYE:
            if (++sTradeData->timer == 80) {
                sTradeData->releasePokeballSpriteId = CreateTradePokeballSprite(sTradeData->monSpriteIds[0],
                                                                                gSprites[sTradeData->monSpriteIds[0]].oam.paletteNum,
                                                                                120, 32, 2, 1, 0x14, 0xfffff);
                sTradeData->state++;
        ...
        case TS_STATE_CROSSING_CREATE_MON_PICS:
            if (!IsMonSpriteNotFlipped(sTradeData->monSpecies[TRADE_PLAYER])) {
                ...
                CalcCenterToCornerVec(&gSprites[sTradeData->monSpriteIds[TRADE_PLAYER]], SPRITE_SHAPE(64x64),
        ...
        case TS_STATE_POKEBALL_ARRIVE:
            ...
            BlendPalettes(1 << (16 + gSprites[sTradeData->bouncingPokeballSpriteId].oam.paletteNum), 16,
                          RGB_WHITEALPHA);
            sTradeData->state++;
            sTradeData->timer = 0;
            break;
        case TS_STATE_FADE_POKEBALL_TO_NORMAL:
            BeginNormalPaletteFade(1 << (16 + gSprites[sTradeData->bouncingPokeballSpriteId].oam.paletteNum), 1, 16, 0,
                                   RGB_WHITEALPHA);
            sTradeData->state++;
            break;
        case TS_STATE_POKEBALL_ARRIVE_WAIT:
            if (gSprites[sTradeData->bouncingPokeballSpriteId].callback == SpriteCallbackDummy) {
                HandleLoadSpecialPokePic_2(&gMonFrontPicTable[sTradeData->monSpecies[TRADE_PARTNER]],
                                           gMonSpritesGfxPtr->sprites.ptr[3], sTradeData->monSpecies[TRADE_PARTNER],
        ...
        case TS_STATE_SHOW_NEW_MON:
            ...
            gSprites[sTradeData->monSpriteIds[TRADE_PARTNER]].y =
            ...
            CreatePokeballSpriteToReleaseMon(sTradeData->monSpriteIds[TRADE_PARTNER],
                                             gSprites[sTradeData->monSpriteIds[TRADE_PARTNER]].oam.paletteNum, 120, 84,
                                             2, 1, 20, 0xFFFFF, sTradeData->monSpecies[TRADE_PARTNER]);
        ...
        case TS_STATE_TRY_EVOLUTION: // Only if in-game trade, link trades use CB2_TryLinkTradeEvolution
            ...
            evoTarget = GetEvolutionTargetSpecies(&gPlayerParty[gSelectedTradeMonPositions[TRADE_PLAYER]],
                                                  EVO_MODE_TRADE, ITEM_NONE);
            if (evoTarget != SPECIES_NONE) {
                TradeEvolutionScene(&gPlayerParty[gSelectedTradeMonPositions[TRADE_PLAYER]], evoTarget,
                                    sTradeData->monSpriteIds[TRADE_PARTNER], gSelectedTradeMonPositions[TRADE_PLAYER]);
            }
            sTradeData->state++;
            break;
...
static void _CreateInGameTradePokemon(u8 whichPlayerMon, u8 whichInGameTrade) {
    ...
    CreateMon(pokemon, inGameTrade->species, level, USE_RANDOM_IVS, TRUE, inGameTrade->personality, OT_ID_PRESET,
              inGameTrade->otId);
...
void CreateInGameTradePokemon(void) {
    ...
    else if (gSpecialVar_0x8004 ==
             7) // Version 2 Step 1 (trade your pokemon for the defined pokemon below and saves your pokemon data to the trader)
    {
        struct Pokemon *pokemon = &gEnemyParty[0];
        CreateMon(pokemon, SPECIES_MEW, 99, 32, FALSE, 0, OT_ID_PRESET,
                  0); // (After the trade this pokemon is set as "Seen and caught" in the players pokedex!)
