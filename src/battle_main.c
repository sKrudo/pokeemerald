    ...
    switch (gBattleCommunication[MULTIUSE_STATE]) {
                ...
                if (gReceivedRemoteLinkPlayers != 0) {
                    if (IsLinkTaskFinished()) {
                        ...
                        *(&gBattleStruct->multiBuffer.linkBattlerHeader.versionSignatureHi) = 3;
                        ...
                        SendBlock(bitmask_all_link_players_but_self(), &gBattleStruct->multiBuffer.linkBattlerHeader,
                                  sizeof(gBattleStruct->multiBuffer.linkBattlerHeader));
        ...
        case 2:
            if ((GetBlockReceivedStatus() & 3) == 3) {
                ...
                gTasks[taskId].data[3] = gBattleStruct->multiBuffer.linkBattlerHeader.vsScreenHealthFlagsLo |
    ...
    switch (gBattleCommunication[MULTIUSE_STATE]) {
        case 0:
            if (gReceivedRemoteLinkPlayers != 0 && IsLinkTaskFinished()) {
                sMultiPartnerPartyBuffer = Alloc(
...
static void EndLinkBattleInSteps(void) {
    ...
    switch (gBattleCommunication[MULTIUSE_STATE]) {
        ...
        case 2:
            ...
            break;
        case 3: CpuFill32(0, (void *) (VRAM), VRAM_SIZE);
...
static void SpriteCB_AnimFaintOpponent(struct Sprite *sprite) {
    ...
    if (--sprite->data[4] == 0) {
        ...
        {
            u8 *dst = gMonSpritesGfxPtr->sprites.byte[GetBattlerPosition(sprite->sBattler)] +
...
static void BattleIntroDrawTrainersOrMonsSprites(void) {
    ...
    for (gActiveBattler = 0; gActiveBattler < gBattlersCount; gActiveBattler++) {
        if ((gBattleTypeFlags & BATTLE_TYPE_SAFARI)
            ...
            gBattleMons[gActiveBattler].type1 = gBaseStats[gBattleMons[gActiveBattler].species].type1;
            gBattleMons[gActiveBattler].type2 = gBaseStats[gBattleMons[gActiveBattler].species].type2;
            gBattleMons[gActiveBattler].ability = GetAbilityBySpecies(gBattleMons[gActiveBattler].species,
                                                                      gBattleMons[gActiveBattler].abilityNum);
        ...
        if (gBattleTypeFlags & BATTLE_TYPE_TRAINER) {
            ...
            if (GetBattlerSide(gActiveBattler) == B_SIDE_OPPONENT) {
                ...
                MarkBattlerForControllerExec(gActiveBattler);
                gBattleResults.lastOpponentSpecies = GetMonData(&gEnemyParty[gBattlerPartyIndexes[gActiveBattler]],
    ...
    for (gActiveBattler = 0; gActiveBattler < gBattlersCount; gActiveBattler++) {
        ...
        switch (gBattleCommunication[gActiveBattler]) {
            ...
            case STATE_WAIT_ACTION_CHOSEN: // Try to perform an action.
                if (!(gBattleControllerExecFlags &
                      ((gBitTable[gActiveBattler]) | (0xF << 28) | (gBitTable[gActiveBattler] << 4) |
                       (gBitTable[gActiveBattler] << 8) | (gBitTable[gActiveBattler] << 12)))) {
                    ...
                    switch (gBattleBufferB[gActiveBattler][1]) {
                        case B_ACTION_USE_MOVE:
                            if (AreAllMovesUnusable()) {
                                ...
                                *(gBattleStruct->stateIdAfterSelScript +
                                  gActiveBattler) = STATE_WAIT_ACTION_CONFIRMED_STANDBY;
                            ...
                            *(gBattleStruct->field_58 + gActiveBattler) = gBattlerPartyIndexes[gActiveBattler];
                            if (gBattleMons[gActiveBattler].status2 & (STATUS2_WRAPPED | STATUS2_ESCAPE_PREVENTION)
                                ...
                                if (gActiveBattler == 2 && gChosenActionByBattler[0] == B_ACTION_SWITCH)
                                    BtlController_EmitChoosePokemon(0, PARTY_ACTION_CHOOSE_MON,
                        ...
                        case B_ACTION_CANCEL_PARTNER:
                            ...
                            if (gBattleMons[GetBattlerAtPosition(
                                    BATTLE_PARTNER(GetBattlerPosition(gActiveBattler)))].status2 & STATUS2_MULTIPLETURNS
                                || gBattleMons[GetBattlerAtPosition(
                                    BATTLE_PARTNER(GetBattlerPosition(gActiveBattler)))].status2 & STATUS2_RECHARGE) {
                                ...
                                RecordedBattle_ClearBattlerAction(
                                    ...
                                    BATTLE_PARTNER(GetBattlerPosition(gActiveBattler)))].noValidMoves
                                           || gDisableStructs[GetBattlerAtPosition(
                                    BATTLE_PARTNER(GetBattlerPosition(gActiveBattler)))].encoredMove)) {
                                RecordedBattle_ClearBattlerAction(
                                        GetBattlerAtPosition(BATTLE_PARTNER(GetBattlerPosition(gActiveBattler))), 1);
                            ...
                            switch (gBattleBufferB[gActiveBattler][1]) {
                                ...
                                default:
                                    ...
                                    if ((gBattleBufferB[gActiveBattler][2] |
                                        ...
                                        *(gBattleStruct->stateIdAfterSelScript +
                                          gActiveBattler) = STATE_WAIT_ACTION_CHOSEN;
                                        return;
                                    } else {
                                        if (!(gBattleTypeFlags & BATTLE_TYPE_PALACE)) {
                                        ...
                                        *(gBattleStruct->chosenMovePositions +
                                        ...
                                        gChosenMoveByBattler[gActiveBattler] = gBattleMons[gActiveBattler].moves[*(
                                                gBattleStruct->chosenMovePositions + gActiveBattler)];
                                        *(gBattleStruct->moveTarget +
                ...
                if (GetBattlerSide(gActiveBattler) == B_SIDE_PLAYER) {
                    if (gBattleResults.playerMon1Species == SPECIES_NONE) {
                        gBattleResults.playerMon1Species = GetMonData(
                                &gPlayerParty[gBattlerPartyIndexes[gActiveBattler]], MON_DATA_SPECIES, NULL);
